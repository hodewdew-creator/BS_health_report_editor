...
/**
 * UI Polish Notes (2025-09):
 * - 상단 섹션 탭(신체검사/치과/종합소견)으로 좌측 패널 한 화면씩만 표시 → 시각적 밀도↓
 * - 우측 결과 패널 sticky 고정(top-24) → 스크롤 시에도 항상 보임
 * - 종합소견: 태그 검색, 선택 태그 트레이, 중분류별 그룹(라벨만, 소프트 톤) → 탐색성↑
 * - 태그 버튼 색상은 절제(연한 톤/선택 시 진한) → 색 과포화↓
 * - 기존 동작(접두사 ⏹, 대분류 설명 제거, [대분류] 헤더 미표시 등) 유지
 */
  ...
  return (
    <Card title="③ 종합 소견" subtitle="검사 선택 → 태그 클릭으로 상세 문구 추가" right={<CopyBtn text={preview} />}>
      ...
      {selectedObjs.length > 0 && (
        <div className="mt-3">
          <div className="text-xs text-slate-500 mb-1">선택된 태그</div>
          <div className="flex flex-wrap gap-2">
            {selectedObjs.map(r => (
              <button key={r._id} onClick={()=> removeTag(String(r._id))}
                className="px-2 py-1 text-xs rounded-lg border bg-slate-100 text-slate-700 border-slate-200 hover:bg-slate-200">
      ...
      {grouped.length > 0 && (
        <div className="mt-4 space-y-3">
          {grouped.map(([sub, rows]) => (
            <div key={sub}>
              <div className="text-[11px] uppercase tracking-wide text-slate-500 mb-1">{sub}</div>
              <div className="flex flex-wrap gap-2">
                {rows.map(row => {
  ...
  return (
    <Card title="도움말" subtitle="설계 목표 & 사용 팁">
      <ul className="list-disc pl-5 space-y-1 text-sm text-slate-700">
        <li>상단 탭으로 섹션을 전환해 한 화면씩 집중해서 편집하세요.</li>
        <li>우측 결과 패널은 스크롤해도 고정되어 항상 복사 가능해요.</li>
        <li>종합소견은 검색·선택 트레이·중분류 그룹으로 빠르게 태그를 찾을 수 있어요.</li>
      </ul>
